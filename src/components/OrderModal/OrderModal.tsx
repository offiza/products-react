...
export const OrderModal: FC<OrderModalProps> = ({ product, onClose }) => {
  ...
  return (
    <div className="ordermodal__background">
      <div className="ordermodal__container">
        ...
        <form onSubmit={handleSubmit}>
          ...
          />
          {name.wasFocused && !name.focus && name.errorMessage && (
            <p className="ordermodal__label">{name.errorMessage}</p>
          )}
          <input
            className="ordermodal__input"
            type="text"
          ...
          />
          {number.wasFocused && !number.focus && number.errorMessage && (
            <p className="ordermodal__label">{number.errorMessage}</p>
          )}
          <button className="ordermodal__button" type="submit">
            <span>Order</span>
          </button>
        </form>
      </div>
      <div className="ordermodal__close" onClick={onClose}></div>
      <div className="ordermodal__close-btn" onClick={onClose}>
        &#10006;
      </div>
    </div>
  );
