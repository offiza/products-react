...
export const CardsPage = () => {
  ...
  return (
    <div style={{ marginTop: '76px auto' }}>
      <CardList products={products} isLoading={isLoading} setCurrentCard={handleSetCard} />
      {currentCard &&
        <OrderModal product={currentCard} onClose={handleClearCard} />
      }
      {!isLoading && <div className='productpage__button' onClick={(event: any) => handleGetCheapest(event)}>Buy Cheapest</div>}
    </div>
  )
}